...
predict_day2_long <- function(new_data, bundle_day2, levels = NULL, vote_threshold = NULL) {
...
predict_day2_wide <- function(new_data, bundle_day2, levels = NULL, vote_threshold = NULL) {
  long <- predict_day2_long(new_data, bundle_day2, levels = levels, vote_threshold = vote_threshold)
...
function(req, res, vote_threshold = NULL, format = "long") {
  ...
  out <- tryCatch(
    {
      if (format == "wide") {
        predict_day2_wide(df, bundle_day2, levels = levels, vote_threshold = vt)
      } else {
        predict_day2_long(df, bundle_day2, levels = levels, vote_threshold = vt)
      }
    },
    error = function(e) e
  )
